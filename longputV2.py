...
st.set_page_config(page_title="Long Put Algorithm",
...
st.markdown("<h1 style='text-align: center; color: black;'>Long Put Algorithm</h1>", unsafe_allow_html=True)
@st.cache_data
...
if purpose_select==purpose[0]:
    build_pos=st.radio("Build long put position?",options=["Yes","no"],index=0) 
    if build_pos=="Yes":
        ...
        st.sidebar.subheader("Long put position builder")
        ...
        if st.radio("Is the last price your entry price?",options=["Yes","No"],index=0) == "No":
            last_price=st.number_input("Enter call contract ask price",min_value=0.00,value=1.00,step=0.05)
        else:
        ...
        st.sidebar.write("Cash available after premium paid:",remainder)
        ...
        if  nr_contracts_buy==0:
            ...
            st.write("The take profit exit price is: ",round(last_price*(1+perc_profit)+last_price,2))
            st.write("The profit amount would be: ",round((last_price*(1+perc_profit)-last_price)*100*nr_contracts_buy,2),"USD including premium paid.")
            st.write("The stop loss price is: ",round(last_price*(1-perc_risk),2))
            st.write("The loss amount would be: ",round((last_price*(1-perc_risk)-last_price)*100*nr_contracts_buy,2),"USD including premium paid.")
...
if purpose_select==purpose[1]:
    ...
    #premium_investment=st.number_input("Enter amount of captial to risk in premium:",min_value=0,step=50,value=200)
    #nr_contracts=int(nr_shares/100)
    st.sidebar.subheader("Long call position check")
    ...
    if st.radio("Is the last price your entry price?",options=["Yes","No"],index=1) == "No":
        entry_price=st.number_input("Enter call contract ask price",min_value=0.00,value=1.00,step=0.05)
    else:
    ...
    #st.write("Based on the price and paramteres selected it is possible to buy up to : USD",last_price*nr_contracts*100,"in premium.")
    st.sidebar.write("Buy to open quantity of contracts:",nr_contracts_buy)
    st.sidebar.write("Premium invested:",round(entry_price*nr_contracts_buy*100,2))
    ...
    st.subheader("Based on data input:")
    if  nr_contracts_buy==0:
        st.write("No contracts held on inventory. To see other scenarios change inputs.")
    else:
        st.write("The take profit exit price is: ",round(entry_price*(1+perc_profit)+entry_price,2))
        st.write("The profit amount would be: ",round((entry_price*(1+perc_profit)-entry_price)*100*nr_contracts_buy,2),"USD including premium paid.")
        st.write("The stop loss price is: ",round(entry_price*(1-perc_risk),2))
        st.write("The loss amount would be: ",round((entry_price*(1-perc_risk)-entry_price)*100*nr_contracts_buy,2),"USD including premium paid.")
        st.write("The running P&L is:",round((live_price-entry_price)*100*nr_contracts_buy,2),"USD including premium paid.")
