...
st.set_page_config(page_title="Cash Secured Put Algorithm",
...
st.markdown("<h1 style='text-align: center; color: black;'>Cash Secured Put Algorithm</h1>", unsafe_allow_html=True)
@st.cache_data
...
def put_loss_on_strike(chain, expiry_price):
    ...
    return put_loss
purpose=["1-Sell to Open Cash Secured Puts",
"2-Check position"]
...
if purpose_select==purpose[0]:
    build_pos=st.radio("Build short put position secured by cash?",options=["Yes","no"],index=0) 
    if build_pos=="Yes":
        #cash_to_secure=st.number_input("Enter amount of captial available to secure put:",min_value=0,step=50,value=200)
        #nr_contracts_sell=int(nr_shares/100)
        st.sidebar.subheader("Cash secured Put position builder")
        ...
        if st.radio("Is the last option price your inital position price?",options=["Yes","No"],index=0) == "No":
            last_price=st.number_input("Enter put contract price received",min_value=0.00,value=1.00,step=0.05)
        else:
            last_price=live_price
        cash_to_secure=st.number_input("Enter amount of captial available to secure put:",min_value=0,step=50,value=round(strike*100,2))
        max_contracts_sell=int(cash_to_secure/(strike*100))
        if max_contracts_sell==0:
            remainder=cash_to_secure
        else: remainder=round(cash_to_secure%(strike*max_contracts_sell*100),2)
        st.write("Possibility to sell up to",max_contracts_sell,"Put contract/s secured by cash. The remainder cash position after securing the contracts is",remainder,"USD.")
        if max_contracts_sell==0:
            st.write("Minimum cash amount required is",round(strike*100,2),"USD.")
        nr_contracts_sell=st.number_input("Enter amount of contracts to sell",min_value=0,max_value=max_contracts_sell,value=max_contracts_sell,step=1)
        ...
        st.sidebar.write("Sell to open quantity of contracts:",nr_contracts_sell)
        st.sidebar.write("Cash required to secure Put:",round(strike*nr_contracts_sell*100,2))
        st.sidebar.write("Cash available after Put is secured:",remainder)

        st.divider()
        st.subheader("P&L potential and targets")
        st.write("Max profit potential is the premium received: ",round(last_price*100*nr_contracts_sell,2), "USD")
        perc_profit=st.number_input("What is the % profit target?",min_value=0,step=5,value=10)
        perc_profit=perc_profit/100
        st.write("Max loss potential is the amount of cash used to secure the contracts: ",strike*100*nr_contracts_sell, "USD")
        ...
        if  nr_contracts_sell==0:
            st.write("No contracts to sell. To see other scenarios change inputs.")
        else:
            st.write("The take profit exit price is: ",round(last_price*(1-perc_profit),2))
            st.write("The profit amount would be: ",round((last_price*(perc_profit)),2)*100*nr_contracts_sell,"USD.")
            st.write("The stop loss price is: ",round(last_price*(1+perc_risk),2))
            st.write("The loss amount would be: ",round((last_price*(perc_risk)),2)*100*nr_contracts_sell,"USD.")


        if st.button("Plot put diagram"):
...
if purpose_select==purpose[1]:
    ...
    #premium_investment=st.number_input("Enter amount of captial to risk in premium:",min_value=0,step=50,value=200)
    #nr_contracts=int(nr_shares/100)
    st.sidebar.subheader("Cash Secured Put position check")
    ...
    if st.button('Score'):
        ...
        sum_score=df_param_score_weights.loc["Score"].dot(df_param_score_weights.loc["Weights"])
        st.write("Total Score is:",round(sum_score,2))
    if st.radio("Is the last price your initial position price?",options=["Yes","No"],index=1) == "No":
        entry_price=st.number_input("Enter put contract price received",min_value=0.00,value=1.00,step=0.05)
    else:
    ...
    nr_contracts_sell=st.number_input("Enter amount of contracts sold",min_value=0,value=1,step=1)
    ...
    st.divider()
    st.subheader("P&L potential and targets")
    st.write("Max profit potential is the premium received: ",round(entry_price*100*nr_contracts_sell,2), "USD")
    perc_profit=st.number_input("What is the % profit target?",min_value=0,step=5,value=10)
    perc_profit=perc_profit/100
    st.write("Max loss potential is the amount of cash used to secure the contracts: ",strike*100*nr_contracts_sell, "USD")
    perc_risk=st.number_input("What is the % risk target?",min_value=0,step=5,value=5)
    ...
    if  nr_contracts_sell==0:
        ...
        st.write("The profit amount would be: ",round((entry_price*(perc_profit)),2)*100*nr_contracts_sell,"USD.")
        st.write("The stop loss price is: ",round(entry_price*(1+perc_risk),2))
        st.write("The loss amount would be: ",round((entry_price*(perc_risk)),2)*100*nr_contracts_sell,"USD.")
        st.write("The current unrealized P&L is ",round(entry_price-live_price,2)*100*nr_contracts_sell,"USD")
